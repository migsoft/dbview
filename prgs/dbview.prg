 ...
 * Miguel Angel Juárez A. - 2009-2023 MigSoft <mig2soft/at/oohg.org>
...
*------------------------------------------------------------------------------*
    ...
    Public cBaseFolder, aTypes, aNewFile := {}, aFtype := {}, aCtrl := {} , aLect := {}
...
*------------------------------------------------------*
   ...
   Elseif cTypeField=='N' .OR. cTypeField=='I' .OR. cTypeField=='W' .OR. cTypeField=='Y' .OR. cTypeField=='B' .OR. cTypeField=='+'
...
*------------------------------------------------------------------------------*
   If !Empty( Alias() )
       ...
       If nOp = 1   // Selecciona registro
          RecReply()
          MuestraRec()
       Else         // Limpia Selecci�n de registro
          nRecCopy := {}
          nRecSel := 0
...
*------------------------------------------------------------------------------*
   If !Empty( Alias() )
      ...
      +'/'+padl(Alltrim(Str(( Alias() )->(LastRec()))),7)+'         Row: '+padl(Alltrim(Str(aRec[1])),7);
      +'   Col:'+padl(Alltrim(Str(aRec[2])),7)
      oWndBase.StatusBar.Item(2) := 'Selected Record: '+if(nRecSel=0,"",Alltrim(Str( nRecSel ,7)))
      oWndBase.StatusBar.Item(3) := 'Index Tag: ' + ( Alias() )->( OrdName() )
      oWndBase.StatusBar.Item(4) := 'Order Key: ' + ( Alias() )->( OrdKey() )
   Else
      oWndBase.StatusBar.Item(1) := ' Record: '
      oWndBase.StatusBar.Item(2) := 'Selected Record: '
      oWndBase.StatusBar.Item(3) := 'Index Tag: '
      oWndBase.StatusBar.Item(4) := 'Order Key: '
   Endif
Return
...
*/
		...
		IF File( cSaveFile )
                   IF !MsgYesNo( PadC("Overwrite existing file ?",Len(cSaveFile)*1.5)+CRLF+cSaveFile, "File already exists." )
                      Return
                   ENDIF
                ENDIF
		...
		IF File( cSaveFile )
                   MsgInfo( cSaveFile, "File Created - "+PROGRAM )
                Else
                   MsgInfo( cSaveFile, "File not Created - "+PROGRAM )
		Endif
...
*--------------------------------------------------------*
   ...
   If !Empty( Alias() )
	...
	Aeval( ( Alias() )->( DBstruct() ), { |e,i| oSheet:Cells( 1, i ):Value := e[DBS_NAME] } )

        For nCell := 1 to ( Alias())->( LastRec() )
            ( Alias())->( DBGoTo( nCell  ) )
            aColumns := ( Alias())->( Scatter() )
...
PRIVATE cFile
  ...
  If File( cSaveFile )
     //MsgInfo( cSaveFile, "File created" )
  Else
     MsgInfo( "Error: ", FError() )
  Endif
